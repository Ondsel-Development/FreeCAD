...
void AssemblyObject::exportASMTFile()
{
    mbdAssembly = makeMbdAssembly();
    objectPartMap.clear();
...
std::shared_ptr<ASMTPart> AssemblyObject::getMbDPart(App::DocumentObject* obj)
{
    ...
    if (it != objectPartMap.end()) {
        ...
        mbdPart = makeMbdPart(obj->getFullName(), plc);
...
std::shared_ptr<ASMTPart> AssemblyObject::makeMbdPart(std::string& name, Base::Placement plc)
{
